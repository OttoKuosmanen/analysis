...
def get_significance_markers(results, alpha):
...
GPT3 = ratings[[col for col in ratings.columns if col.lower().startswith('gpt3_')]] # This model is actually, part of the GPT3.5 series text davinci 3
...
data_t = {
    'Models': ['Davinci-3', 'GPT-3.5-Turbo', 'GPT-4'], # Names changed to more specific
...
significance_markers = get_significance_markers(results, 0.0125)
...
ai_bars = plt.bar(x_pos, percentages, color='#034f84', label='AI Chosen', width=0.66)
...
for i in range(len(models)):
    complement_bar = plt.bar(x_pos[i], complement_percentages[i], bottom=percentages[i], 
                             color='#f7786b', edgecolor='none', label='Human Chosen' if i == 0 else "", width =0.66)
...
for i, bar in enumerate(ai_bars):
    ...
    y = 50
    plt.text(x, y, significance_markers[i], ha='center', va='bottom', fontsize=24, color="white")


# Create custom handles for the legend
legend_handles = [
    Patch(facecolor='#f7786b', label='Human Chosen'),
    Patch(facecolor='#034f84', label='AI Chosen')
]
...
plt.xticks(x_pos, models, fontweight='bold', fontsize=18)

plt.ylabel('Chosen as More Helpful Advice', fontsize=18, fontweight='bold')
plt.title('What Advice is More Helpful: Percentage Distribution of Choice', fontsize=18, fontweight='bold', pad=12)
...
for comment in data_ore['Comment']:
    print(comment)
